...
public class FolderControllerTest extends BaseWebTest {
    ...
    @Test
    public void should_return_full_list_of_files_by_hearing_resource() throws Exception {
        var folderName = "audioStream123";
        var hearingResource = "CVP";
        var fileName1 = "32123-32-23-332.mpeg";
        var fileName2 = "dcfds9923-ss-FB.mpeg";
        Set<String> folderSet = Set.of(fileName1, fileName2);
        when(folderService.getStoredFiles(folderName,hearingResource)).thenReturn(folderSet);
        mockMvc.perform(get("/" + hearingResource + "/folders/" + folderName))
            .andDo(print())
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.folder-name").value(folderName))
            .andExpect(jsonPath("$.filenames", hasSize(2)))
    ...
    @Test
    public void should_return_empty_list_of_files_by_hearing_resource() throws Exception {
        var folderName = "audioStream9084";
        var hearingResource = "CVP";
        Set<String> folderSet = Set.of();
        when(folderService.getStoredFiles(folderName, hearingResource)).thenReturn(folderSet);
